...
mod generators {
    ...
    pub(crate) fn cycle(n: usize) -> Result<NormTree, &'static str> {
        if n < 1 {
    ...
    pub(crate) fn sonic(n: usize) -> Result<HashMap<String, Vec<String>>, &'static str> {
        if n < 1 || 9 < n {
...
static SONIC_3_TEXT: &'static str = "\
+-x1-+----+
+----+-22 |
|         +-32-+
+--------------+-33\
...
mod tests {
    ...
    fn nodes_contains_exact() {
        ...
        let expected_nodes = vec![0, 1, 2, 3, 4];
...
static PADDINGS: &'static str = "          ";
static HORIZONTALS: &'static str = "----------";

static NOTHING: &'static str = "";
static PADDING: &'static str = " ";
static JUNCTION: &'static str = "+";
static VERTICAL: &'static str = "|";
static HORIZONTAL: &'static str = "-";

fn normalized<T: Eq + Hash + Debug>(tree: &HashMap<T, Vec<T>>, order: &Vec<T>) -> NormTree {
